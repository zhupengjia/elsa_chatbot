...
from .nltk_sentiment import NLTK_Sentiment
from .dialog_status import Dialog_Status
...
class InteractSession:
    def __init__(self, vocab, tokenizer, ner, topic_manager, sentiment_analyzer, max_seq_len=100, max_entity_types=1024, device='cpu', logger=None, **args):
        '''
    ...
    def build(cls, config):
        ...
        sentiment_analyzer = NLTK_Sentiment()
        ...
        for skill_name in config.skills:
            ...
            if "hook" in config.skills[skill_name]:
                if not hasattr(Hooks, config.skills[skill_name].hook.wrapper):
                    raise RuntimeError("Error!! Hook {} not implemented!".format(config.skills[skill_name].hook.wrapper))
                hook_cls = getattr(Hooks, config.skills[skill_name].hook.wrapper)
                if not "parameters"  in config.skills[skill_name].hook:
                    hook = hook_cls()
                else:
                    hook = hook_cls(**config.skills[skill_name].hook.parameters) 
                response_params.pop('hook')
            else:
                hook = None
            response = skill_cls(tokenizer=tokenizer, vocab=vocab, hook=hook, max_seq_len=config.model.max_seq_len, **response_params)
            response.init_model(saved_model=config.skills[skill_name].saved_model, device=config.model.device, skill_name=skill_name, encoder=encoder)
            topic_manager.register(skill_name, response)
        
        return cls(vocab=vocab, tokenizer=tokenizer, ner=ner, topic_manager=topic_manager, logger=logger, sentiment_analyzer=sentiment_analyzer, **config.model)

    
    def new_dialog(self):
        return Dialog_Status.new_dialog(self.vocab, self.tokenizer, self.ner, self.topic_manager, self.sentiment_analyzer, self.max_seq_len, self.max_entity_types)
