...
class GenerativeResponse(SkillBase):
    ...
    def init_model(self, saved_model="generative_tracker.pt", device='cpu', gpu_ids=None, **args):
        ...
        if os.path.exists(saved_model):
            ...
            copy_args("entity_layers", "hidden", "entity_layers")
            copy_args("entity_emb_dim", "hidden", "entity_emb_dim")
            copy_args("max_entity_types", "hidden", "max_entity_types")
    ...
    def update_response(self, response, current_status):
        ...
        response_string = self.tokenizer.tokens2sentence(self.vocab.id2words(response[response_mask.astype("bool_")][1:-1]))
        
        response_string = self.entity_replace(response_string, current_status.keys())
        response_string = response_string.format(**current_status)

        current_status['$RESPONSE'] = response_string
